...
function ZonePill({ relation }) {
  return (
    <span className={`text-xs font-bold rounded-full px-2.5 py-0.5 ring-1
      ${relation === 'intra' ? 'bg-blue-100 text-blue-700 ring-blue-200' : 'bg-orange-100 text-orange-700 ring-orange-200'}`}>
...
function RateCardForm({ editing, onSubmit, onCancel, working }) {
  const [form, setForm] = useState(editing
    ? { orderType: editing.orderType, zoneRelation: editing.zoneRelation, baseRate: editing.baseRate, ratePerKg: editing.ratePerKg, effectiveFrom: editing.effectiveFrom?.split('T')[0] || '', isActive: editing.isActive }
    : EMPTY_CARD);
  useEffect(() => { setForm(editing ? { orderType: editing.orderType, zoneRelation: editing.zoneRelation, baseRate: editing.baseRate, ratePerKg: editing.ratePerKg, effectiveFrom: editing.effectiveFrom?.split('T')[0] || '', isActive: editing.isActive } : EMPTY_CARD); }, [editing]);
  function set(k, v) { setForm(f => ({ ...f, [k]: v })); }
  const example = form.baseRate && form.ratePerKg ? (parseFloat(form.baseRate) + 5 * parseFloat(form.ratePerKg)).toFixed(2) : null;
    ...
    <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select className="input text-sm" value={form.orderType} onChange={e => set('orderType', e.target.value)}>
            <option value="B2C">B2C — Consumer</option><option value="B2B">B2B — Business</option>
          </select>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input className="input text-sm" type="number" min="0" step="0.01" required placeholder="e.g. 50.00"
            value={form.baseRate} onChange={e => set('baseRate', e.target.value)} />
          <p className="field-hint">Fixed charge per shipment.</p>
        </div>
        <div>
          <label className="label">Rate per kg (₹) <span className="text-red-400">*</span></label>
          <input className="input text-sm" type="number" min="0" step="0.01" required placeholder="e.g. 10.00"
            value={form.ratePerKg} onChange={e => set('ratePerKg', e.target.value)} />
          <p className="field-hint">Multiplied by chargeable weight.</p>
        </div>
      </div>
      {example && (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input className="input text-sm" type="date" value={form.effectiveFrom} onChange={e => set('effectiveFrom', e.target.value)} />
          <p className="field-hint">Most recent active card is used.</p>
        </div>
        <div className="flex items-center gap-2 pt-7">
          <input id="rc-active" type="checkbox" className="w-4 h-4 rounded accent-brand-600"
            checked={form.isActive} onChange={e => set('isActive', e.target.checked)} />
          <label htmlFor="rc-active" className="text-sm font-semibold text-gray-700 cursor-pointer">Active</label>
        </div>
      </div>
      <div className="flex gap-2 pt-1">
        <button type="submit" className="btn-primary flex-1" disabled={working}>
          {working ? <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Saving…</> : editing ? '✓ Update Rate Card' : '+ Add Rate Card'}
        </button>
...
function CodRuleForm({ editing, onSubmit, onCancel, working }) {
  const [form, setForm] = useState(editing ? { orderType: editing.orderType, surchargeType: editing.surchargeType, value: editing.value, isActive: editing.isActive } : EMPTY_COD);
  useEffect(() => { setForm(editing ? { orderType: editing.orderType, surchargeType: editing.surchargeType, value: editing.value, isActive: editing.isActive } : EMPTY_COD); }, [editing]);
  function set(k, v) { setForm(f => ({ ...f, [k]: v })); }
  const exSurcharge = form.value
    ? form.surchargeType === 'flat' ? parseFloat(form.value).toFixed(2)
    : ((parseFloat(form.value) / 100) * 300).toFixed(2) : null;
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select className="input text-sm" value={form.surchargeType} onChange={e => set('surchargeType', e.target.value)}>
            <option value="flat">Flat amount (₹)</option><option value="percent">Percentage (%)</option>
          </select>
        </div>
      </div>
      <div>
        <label className="label">Value <span className="text-red-400">*</span>
          <span className="ml-1 text-xs font-normal text-gray-400">{form.surchargeType === 'flat' ? '— ₹ amount' : '— e.g. 2 for 2%'}</span>
        </label>
        <div className="relative">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 text-sm font-bold">
            {form.surchargeType === 'flat' ? '₹' : '%'}
          </span>
          <input className="input text-sm pl-8" type="number" min="0" step="0.01" required
      ...
      <div className="flex items-center gap-2">
        <input id="cod-active" type="checkbox" className="w-4 h-4 rounded accent-brand-600"
          checked={form.isActive} onChange={e => set('isActive', e.target.checked)} />
        <label htmlFor="cod-active" className="text-sm font-semibold text-gray-700 cursor-pointer">Active</label>
      </div>
      <div className="flex gap-2 pt-1">
        <button type="submit" className="btn-primary flex-1" disabled={working}>
          {working ? <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Saving…</> : editing ? '✓ Update COD Rule' : '+ Add COD Rule'}
        </button>
        {editing && <button type="button" className="btn-secondary" onClick={onCancel}>Cancel</button>}
      </div>
    </form>
...
export default function AdminRateCards() {
  ...
  function fetchAll() { return Promise.all([api.get('/admin/rate-cards'), api.get('/admin/cod-rules')]).then(([c, r]) => { setCards(c); setCodRules(r); }); }
  useEffect(() => { fetchAll().catch(err => showMsg('error', err.message)).finally(() => setLoading(false)); }, []);

  async function handleCardSubmit(form) {
    setCardWorking(true);
    try {
      if (editingCard) { await api.put(`/admin/rate-cards/${editingCard.id}`, form); showMsg('success', 'Rate card updated.'); }
      else { await api.post('/admin/rate-cards', form); showMsg('success', 'Rate card created.'); }
      setEditingCard(null); await fetchAll();
    } catch (err) { showMsg('error', err.message); }
    finally { setCardWorking(false); }
  }
  async function handleCodSubmit(form) {
    setCodWorking(true);
    try {
      if (editingCod) { await api.put(`/admin/cod-rules/${editingCod.id}`, form); showMsg('success', 'COD rule updated.'); }
      else { await api.post('/admin/cod-rules', form); showMsg('success', 'COD rule created.'); }
      setEditingCod(null); await fetchAll();
    } catch (err) { showMsg('error', err.message); }
    finally { setCodWorking(false); }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <PageHeader icon="💰" title="Rate Cards & COD Rules"
        description="All rates are admin-controlled. The most recent active card matching order type and zone relation is applied." />
      ...
      <div className="mb-6 flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl
        ...
        <div className="text-sm text-blue-800 space-y-1">
          ...
          <p className="text-blue-700 leading-relaxed">
            <strong>Volumetric wt</strong> = (L×B×H)/5000 &nbsp;·&nbsp;
            <strong>Chargeable wt</strong> = max(actual, volumetric) &nbsp;·&nbsp;
            <strong>Base charge</strong> = base_rate + (chargeable_wt × rate_per_kg) &nbsp;·&nbsp;
            COD surcharge added on top if payment type is COD.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        <div className="card bg-gradient-to-br from-orange-50/50 to-white border-orange-100">
          <div className="flex items-center gap-2 mb-5 pb-3 border-b border-orange-100">
            <span className="w-7 h-7 rounded-xl bg-orange-100 text-orange-700 flex items-center justify-center text-sm shrink-0 ring-1 ring-orange-200">💵</span>
            <h2 className="font-bold text-gray-800">{editingCod ? 'Editing COD Rule' : 'Add COD Surcharge Rule'}</h2>
          </div>
          <CodRuleForm editing={editingCod} onSubmit={handleCodSubmit} onCancel={() => setEditingCod(null)} working={codWorking} />
        </div>
      </div>
      ...
      <div className="card mb-6 shadow-card-md">
        ...
        {cards.length === 0 ? (
          <EmptyState icon="📊" title="No rate cards yet" compact
            description="Add your first rate card above. Without an active card, orders cannot be quoted." />
        ) : (
          <div className="table-container">
            <table className="table">
              <thead className="table-head">
                <tr>{['Order Type','Zone','Base Rate','Per kg','5 kg Example','Effective','Status',''].map(h => <th key={h} className="table-th">{h}</th>)}</tr>
              </thead>
      ...
      <div className="card shadow-card-md">
        ...
        {codRules.length === 0 ? (
          <EmptyState icon="💵" title="No COD rules yet" compact
            description="Without a COD rule, Cash on Delivery orders carry no surcharge." />
        ) : (
          <div className="table-container">
            <table className="table">
              <thead className="table-head">
                <tr>{['Order Type','Type','Value','₹300 Example','Status',''].map(h => <th key={h} className="table-th">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-surface-100">
                {codRules.map((r, idx) => {
