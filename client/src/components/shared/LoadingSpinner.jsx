  ...
  const content = (
    <div className="flex flex-col items-center justify-center gap-4 py-16 animate-fade-in">
      <div className="relative w-14 h-14">
        {/* Track ring */}
        <div className="absolute inset-0 w-14 h-14 rounded-full border-4 border-brand-100" />
        {/* Spinning ring */}
        <div className="absolute inset-0 w-14 h-14 rounded-full border-4 border-brand-600
...
export function Skeleton({ className = 'h-4 w-full' }) {
  return (
    <div className={`rounded-xl bg-surface-200 animate-pulse ${className}`} />
  );
}
            ...
            <tr key={r} className={`border-b border-gray-100 last:border-0 ${r % 2 === 1 ? 'bg-surface-50/60' : ''}`}>
              {Array.from({ length: cols }).map((_, c) => (
                <td key={c} className="table-td">
                  <div className={`h-3.5 rounded-lg bg-surface-200 ${c === 0 ? 'w-20' : c === cols - 1 ? 'w-16' : 'w-full'}`} />
                </td>
              ))}
            </tr>
          ))}
